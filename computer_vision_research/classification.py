                ...
                featureVectors.append(
                    np.array(
                        [
                            ...
                            trackedObjects[j].history[i +
                                                      midstep].X, trackedObjects[j].history[i+midstep].Y,
                            trackedObjects[j].history[i +
                                                      step].X, trackedObjects[j].history[i+step].Y
                        ]
                    )
                )
...
def make_feature_vectors_version_two(trackedObjects: List, k: int, labels: np.ndarray):
    ...
    y_newLabels = []
    # [start_time, mid_time, end_time, history_length, trackID]
    featurevector_metadata = []
    ...
    for i, t in tqdm.tqdm(enumerate(trackedObjects), desc="Features for classification.", total=len(trackedObjects)):
        ...
        for j in range(0, t.history_X.shape[0]-max_stride):
            ...
            feature_vector = insert_weights_into_feature_vector(
                midx, end_idx, n_weights, t.history_X, t.history_Y, 2, feature_vector)
            feature_vector = insert_weights_into_feature_vector(
                midx, end_idx, n_weights, t.history_VX_calculated, t.history_VY_calculated, 2, feature_vector)
            if X_feature_vectors.shape == (0,):
                X_feature_vectors = np.array(
                    feature_vector).reshape((-1, 4+(n_weights*4)))
            else:
...
def level_features(X: np.ndarray, y: np.ndarray, ratio_to_min: float = 2.0):
    ...
    if X.shape[0] != y.shape[0]:
        raise ValueError(
            "Number of samples and number of labels should be equal.")
    labels = list(set(y))
    label_counts = np.zeros(shape=(len(labels)),
                            dtype=int)  # init counter vector
    y = y.astype(int)
    for y_ in y:
        label_counts[y_] += 1
    # find min count
...
def data_preprocessing_for_classifier(path2db: str, min_samples=10, max_eps=0.2, xi=0.1, min_cluster_size=10, n_jobs=18, from_half=False, features_v2=False, features_v2_half=False, features_v3=False):
    ...
    labels = optics_clustering_on_nx4(filteredTracks, min_samples=min_samples, max_eps=max_eps,
                                      xi=xi, min_cluster_size=min_cluster_size, n_jobs=n_jobs, path2db=path2db, threshold=thres)

    if from_half:
        X, y, metadata = make_feature_vectors_version_one_half(
            filteredTracks, 6, labels)
    elif features_v2:
        X, y, metadata = make_feature_vectors_version_two(
            filteredTracks, 6, labels)
    elif features_v2_half:
        X, y, metadata = make_feature_vectors_version_two_half(
            filteredTracks, 6, labels)
    elif features_v3:
        X, y, metadata = make_feature_vectors_version_three(
            filteredTracks, 6, labels)
    else:
        X, y, metadata = make_feature_vectors_version_one(
            filteredTracks, 6, labels)
...
def data_preprocessing_for_calibrated_classifier(path2db: str, min_samples=10, max_eps=0.2, xi=0.1, min_cluster_size=10, n_jobs=18):
    ...
    filteredTracks = filter_trajectories(tracks, threshold=thres)
    filteredTracks = filter_by_class(filteredTracks)
    labels = optics_clustering_on_nx4(filteredTracks, min_samples=min_samples, max_eps=max_eps, xi=xi,
                                      min_cluster_size=min_cluster_size, path2db=path2db, threshold=thres, n_jobs=n_jobs, show=True)
...
def data_preprocessing_for_classifier_from_joblib_model(model, min_samples=10, max_eps=0.2, xi=0.15, min_cluster_size=10, n_jobs=18, from_half=False, features_v2=False, features_v2_half=False, features_v3=False):
    ...
    labels = optics_on_featureVectors(featureVectors, min_samples=min_samples,
                                      xi=xi, min_cluster_size=min_cluster_size, max_eps=max_eps, n_jobs=n_jobs)

    if from_half:
        X, y, metadata = make_feature_vectors_version_one_half(
            model.tracks, 6, labels)
    elif features_v2:
        X, y, metadata = make_feature_vectors_version_two(
            model.tracks, 6, labels)
    elif features_v2_half:
...
def preprocess_dataset_for_training(path2dataset: str, min_samples=10, max_eps=0.2, xi=0.15, min_cluster_size=10, n_jobs=18, cluster_features_version: str = "4D", threshold: float = 0.4, classification_features_version: str = "v1", stride: int = 15, level: float = None, n_weights: int = 3, weights_preset: int = 1, p_norm: int = 2):
    ...
    labels = optics_on_featureVectors(featureVectors, min_samples=min_samples, xi=xi,
                                      min_cluster_size=min_cluster_size, max_eps=max_eps, n_jobs=n_jobs, p=p_norm)
    ...
    if classification_features_version == "v1":
        ...
        X, y, metadata = make_feature_vectors_version_three_half(
            tracks, 6, labels)
    elif classification_features_version == "v4":
        X, y, metadata = make_feature_vectors_version_four(
            tracks, stride, labels)
    elif classification_features_version == "v5":
        X, y, metadata = make_feature_vectors_version_five(
            tracks, labels, stride, n_weights)
        ...
        X, y, metadata = make_feature_vectors_version_five(
            tracks, labels, stride, weights_presets[weights_preset])
    elif classification_features_version == "v7":
        X, y, metadata = make_feature_vectors_version_seven(
            tracks, labels, stride)
...
def KNNClassification(X: np.ndarray, y: np.ndarray, n_neighbours: int):
    ...
    classifier = KNeighborsClassifier(
        n_neighbors=n_neighbours, weights='distance').fit(X, y)
    return classifier
...
def VotingClassification(X: np.ndarray, y: np.ndarray):
    ...
    classifier = VotingClassifier(
        estimators=[('knn', clf1), ('sgd', clf2), ('gp', clf3),
                    ('gnb', clf4), ('mlp', clf5)]
    ).fit(X, y)
    return classifier
...
def ValidateClassification(clfmodel, X_valid: np.ndarray, y_valid: np.ndarray):
    ...
    print(
        f"Number of mislabeled points out of a total {X_valid.shape[0]} points : {(y_valid != y_predict).sum()} \nAccuracy: {(1-((y_valid != y_predict).sum() / X_valid.shape[0])) * 100} %")
...
def CalibratedClassificationWorker(path2db: str, **argv):
    ...
    for cls in models:
        ...
        calibrated = CalibratedClassifierCV(
            models[cls], method="sigmoid", n_jobs=18).fit(X_train, y_train)
...
def BinaryClassificationWorkerTrain(path2db: str, path2model=None, **argv):
    ...
    if path2model is not None:
        ...
        X_train, y_train, metadata_train, X_valid, y_valid, metadata_valid, tracks = data_preprocessing_for_classifier(path2db, min_samples=argv['min_samples'],
                                                                                                                       ...
                                                                                                                       min_cluster_size=argv[
                                                                                                                           'min_cluster_size'],
                                                                                                                       n_jobs=argv['n_jobs'], from_half=argv[
                                                                                                                           'from_half'],
                                                                                                                       features_v2=argv[
                                                                                                                           'features_v2'],
                                                                                                                       features_v2_half=argv[
    ...
    if not os.path.isdir(os.path.join('research_data', path2db.split('/')[-1].split('.')[0], "tables")):
        os.mkdir(os.path.join('research_data', path2db.split(
            '/')[-1].split('.')[0], "tables"))
    savepath = os.path.join(os.path.join(
        'research_data', path2db.split('/')[-1].split('.')[0], "tables"))

    for clr in tqdm(models, desc="Classifier trained."):
        binaryModel = OneVSRestClassifierExtended(models[clr](
            **parameters[clr]), tracks, n_jobs=argv['n_jobs'])
        ...
        top_picks = []
        for i in range(1, 4):
            top_picks.append(binaryModel.validate_predictions(
                X_valid, y_valid, top=i))
        balanced_threshold = binaryModel.validate(
            X_valid, y_valid, argv['threshold'])
        ...
        with pd.ExcelWriter(filename) as writer:
            probability_over_time.to_excel(
                writer, sheet_name="Probability_over_time")  # each feature vector
        ...
        if argv['from_half']:
            save_model(path2db, str(
                "binary_"+clr+strfy_dict_params(parameters[clr])+"_from_half"), binaryModel)
        elif argv['features_v2']:
            save_model(path2db, str("binary_"+clr +
                       strfy_dict_params(parameters[clr])+"_v2"), binaryModel)
        elif argv['features_v2_half']:
            save_model(path2db, str(
                "binary_"+clr+strfy_dict_params(parameters[clr])+"_v2_from_half"), binaryModel)
        elif argv['features_v3']:
            save_model(path2db, str("binary_"+clr +
                       strfy_dict_params(parameters[clr])+"_v3"), binaryModel)
...
def train_binary_classifiers(path2dataset: str, outdir: str, **argv):
    ...
    for clr in tqdm(models, desc="Classifier trained."):
        binaryModel = OneVSRestClassifierExtended(models[clr](
            **parameters[clr]), tracks_labels, n_jobs=argv['n_jobs'])
        ...
        if argv['batch_size'] is not None:
            try:
                ...
                for X_batch, y_batch in iter_minibatches(X, y, batch_size):
                    ...
                    binaryModel.partial_fit(
                        X_batch, y_batch, classes=all_classes, centroids=cluster_centroids)
                    iteration += 1
                ...
                print(
                    f"Training without minibatches. Batchsize is: {X.shape[0]:10d}\n")
        ...
        if argv['classification_features_version'] == 'v1':
            save_model(outdir, str("binary_"+clr +
                       strfy_dict_params(parameters[clr])), binaryModel)
        elif argv['classification_features_version'] == 'v1_half':
            save_model(outdir, str(
                "binary_"+clr+strfy_dict_params(parameters[clr])+"_from_half"), binaryModel)
            ...
            save_model(outdir, str(
                "binary_"+clr+strfy_dict_params(parameters[clr])+"_v3_from_half"), binaryModel)
        elif argv['classification_features_version'] == 'v4':
            save_model(outdir, str("binary_"+clr +
                       strfy_dict_params(parameters[clr])+"_v4"), binaryModel)
        elif argv['classification_features_version'] == 'v5':
            save_model(outdir, str(
                "binary_"+clr+strfy_dict_params(parameters[clr])+f"_{argv['n_weights']}_v5"), binaryModel)
        elif argv['classification_features_version'] == 'v6':
            save_model(outdir, str(
                "binary_"+clr+strfy_dict_params(parameters[clr])+f"_{argv['n_weights']}_v6"), binaryModel)
        elif argv['classification_features_version'] == 'v7':
            save_model(outdir, str("binary_"+clr+strfy_dict_params(
                parameters[clr])+f"_stride-{argv['stride']}_v7"), binaryModel)
...
def BinaryClassificationTrain(classifier: str, path2db: str, **argv):
    ...
    # print out pandas dataframe in markdown table format.
    print(table.to_markdown())
...
def BinaryDecisionTreeClassification(path2dataset: str, min_samples: int, max_eps: float, xi: float, min_cluster_size: int, n_jobs: int, from_half=False):
    ...
    for d in range(2, 11):
        ...
        # Validating the predict_proba() mathod, that returns back probability for every class
        predict_proba_balanced_accuracy = binaryModel.validate(
            X_valid, y_valid, threshold=threshold)
        # validating predict() method, that returns only the highest predicted class
        predict_accuracy = binaryModel.validate_predictions(
            X_valid, y_valid, threshold=threshold)
        # Create tables for accurcy
        table_one[f"Depth {d}"] = predict_proba_balanced_accuracy
        table_one.loc[0, f"Depth {d} multiclass average"] = np.average(
            predict_proba_balanced_accuracy)
...
def validate_models(path2models: str, **argv):
    ...
    for clr, m in zip(classifier_names, models):
        ...
        filename = os.path.join(
            savepath, f"{datetime.date.today()}_{clr}.xlsx")
        with pd.ExcelWriter(filename) as writer:
            probability_over_time.to_excel(
                writer, sheet_name="Probability_over_time")
...
def investigateRenitent(path2model: str, threshold: float, **argv):
    ...
    renitent_vector = true_class_under_threshold(
        probas, y_test, X_test, threshold)

    renitent_vector_2 = all_class_under_threshold(
        probas, y_test, X_test, threshold)
    ...
    if len(renitent_vector) > 0:
        ax[0].set_title(
            f"Renitent: true class under threshold {threshold}: {len(renitent_vector)}")
        ax[0].scatter(renitent_vector[:, 0], 1 -
                      renitent_vector[:, 1], s=2.5, c='g')
        ax[0].scatter(renitent_vector[:, 4], 1 - renitent_vector[:, 5], s=2.5)
        ax[0].scatter(renitent_vector[:, 6], 1 -
                      renitent_vector[:, 7], s=2.5, c='r')
        print(
            f"There are {len(renitent_vector)} renitent detections out of {len(X_test)}.")
    else:
        print(f"Renitent: true class under threshold {threshold}")

    if len(renitent_vector_2) > 0:
        ax[1].set_title(
            f"Renitent: classes under threshold {threshold}: {len(renitent_vector_2)}")
        ax[1].scatter(renitent_vector_2[:, 0], 1 -
                      renitent_vector_2[:, 1], s=2.5, c='g')
        ax[1].scatter(renitent_vector_2[:, 4], 1 -
                      renitent_vector_2[:, 5], s=2.5)
        ax[1].scatter(renitent_vector_2[:, 6], 1 -
                      renitent_vector_2[:, 7], s=2.5, c='r')
        print(
            f"There are {len(renitent_vector_2)} renitent detections out of {len(X_test)}.")
    else:
...
def plot_decision_tree(path2model: str):
    ...
    for i, m in enumerate(model.estimators_):
        plot_name = Path(path2model).stem + f"_decision_tree_{i}.gv"
...
def cross_validate(path2dataset: str, outputPath: str = None, train_ratio=0.75, seed=1, n_splits=5, n_jobs=18, estimator_params_set=1, classification_features_version: str = "v1", stride: int = 15, level: float = None, n_weights: int = 3, weights_preset: int = 1, threshold: float = 0.7, **estkwargs):
    ...
    train, test = train_test_split(
        train_dict, train_size=train_ratio, random_state=seed)
    ...
    if classification_features_version == "v1":
        X_train, y_train, metadata_train = make_feature_vectors_version_one(
            trackedObjects=tracks_train, k=6, labels=labels_train)
        X_test, y_test, metadata_train = make_feature_vectors_version_one(
            trackedObjects=tracks_test, k=6, labels=labels_test)
    elif classification_features_version == "v1_half":
        X_train, y_train, metadata_train = make_feature_vectors_version_one_half(
            trackedObjects=tracks_train, k=6, labels=labels_train)
        X_test, y_test, metadata_train = make_feature_vectors_version_one_half(
            trackedObjects=tracks_test, k=6, labels=labels_test)
    elif classification_features_version == "v2":
        X_train, y_train, metadata_train = make_feature_vectors_version_two(
            trackedObjects=tracks_train, k=6, labels=labels_train)
        X_test, y_test, metadata_train = make_feature_vectors_version_two(
            trackedObjects=tracks_test, k=6, labels=labels_test)
    elif classification_features_version == "v2_half":
        X_train, y_train, metadata_train = make_feature_vectors_version_two_half(
            trackedObjects=tracks_train, k=6, labels=labels_train)
        X_test, y_test, metadata_train = make_feature_vectors_version_two_half(
            trackedObjects=tracks_test, k=6, labels=labels_test)
    elif classification_features_version == "v3":
        X_train, y_train, metadata_train = make_feature_vectors_version_three(
            trackedObjects=tracks_train, k=6, labels=labels_train)
        X_test, y_test, metadata_train = make_feature_vectors_version_three(
            trackedObjects=tracks_test, k=6, labels=labels_test)
        cluster_centroids = aoiextraction([t["track"] for t in tracks_filtered], [
                                          t["class"] for t in tracks_filtered])
        fit_params = {
            'centroids': cluster_centroids
        }
    elif classification_features_version == "v3_half":
        X_train, y_train, metadata_train = make_feature_vectors_version_three_half(
            trackedObjects=tracks_train, k=6, labels=labels_train)
        X_test, y_test, metadata_train = make_feature_vectors_version_three_half(
            trackedObjects=tracks_test, k=6, labels=labels_test)
        cluster_centroids = aoiextraction([t["track"] for t in tracks_filtered], [
                                          t["class"] for t in tracks_filtered])
        fit_params = {
        ...
        X_train, y_train, metadata_train = make_feature_vectors_version_four(
            trackedObjects=tracks_train, max_stride=stride, labels=labels_train)
        X_test, y_test, metadata_train = make_feature_vectors_version_four(
            trackedObjects=tracks_test, max_stride=stride, labels=labels_test)
    elif classification_features_version == "v5":
        X_train, y_train, metadata_train = make_feature_vectors_version_five(
            trackedObjects=tracks_train, labels=labels_train, max_stride=stride, n_weights=n_weights)
        X_test, y_test, metadata_test = make_feature_vectors_version_five(
            trackedObjects=tracks_test, labels=labels_test, max_stride=stride, n_weights=n_weights)
    elif classification_features_version == "v6":
        weights_presets = {
            1: np.array([1.0, 1.0, 1.0, 1.0, 1.5, 1.5, 1.5, 1.5, 2.0, 2.0, 2.0, 2.0]),
            2: np.array([1.0, 1.0, 1.0, 1.0, 2.0, 2.0, 2.0, 2.0, 3.0, 3.0, 3.0, 3.0])
        }
        X_train, y_train, metadata_train = make_feature_vectors_version_six(
            trackedObjects=tracks_train, labels=labels_train, max_stride=stride, weights=weights_presets[weights_preset])
        X_test, y_test, metadata_test = make_feature_vectors_version_six(
            trackedObjects=tracks_test, labels=labels_test, max_stride=stride, weights=weights_presets[weights_preset])
    if level is not None:
        X_train, y_train = level_features(X_train, y_train, level)
        X_test, y_test = level_features(X_test, y_test, level)
    elif classification_features_version == "v7":
        X_train, y_train, _ = make_feature_vectors_version_seven(
            trackedObjects=tracks_train, labels=labels_train, max_stride=stride)
        X_test, y_test, _ = make_feature_vectors_version_seven(
            trackedObjects=tracks_test, labels=labels_test, max_stride=stride)
    ...
    splits = np.append(np.arange(1, 6, 1), [
                       "Max split", "Mean", "Standart deviation"])
    ...
    for m in tqdm(models, desc="Cross validate models"):
        clf_ovr = OneVSRestClassifierExtended(estimator=models[m](
            **parameters[estimator_params_set-1][m]), tracks=tracks_train, n_jobs=n_jobs)

        basic_scores = cross_val_score(
            clf_ovr, X_train, y_train, cv=n_splits, fit_params=fit_params, n_jobs=n_jobs)
        basic_table[m] = np.append(basic_scores, [np.max(
            basic_scores), basic_scores.mean(), basic_scores.std()])

        balanced_scores = cross_val_score(
            clf_ovr, X_train, y_train, cv=n_splits, scoring='balanced_accuracy', fit_params=fit_params, n_jobs=n_jobs)
        balanced_table[m] = np.append(balanced_scores, [np.max(
            balanced_scores), balanced_scores.mean(), balanced_scores.std()])

        top_k_scores = cross_validate(
            clf_ovr, X_train, y_train, scoring=top_k_scorers, cv=5, fit_params=fit_params, n_jobs=n_jobs)
        top_1_table[m] = np.append(top_k_scores['test_top_1'], [np.max(
            top_k_scores['test_top_1']), top_k_scores['test_top_1'].mean(), top_k_scores['test_top_1'].std()])
        top_2_table[m] = np.append(top_k_scores['test_top_2'], [np.max(
            top_k_scores['test_top_2']), top_k_scores['test_top_2'].mean(), top_k_scores['test_top_2'].std()])
        top_3_table[m] = np.append(top_k_scores['test_top_3'], [np.max(
            top_k_scores['test_top_3']), top_k_scores['test_top_3'].mean(), top_k_scores['test_top_3'].std()])
        ...
        final_top_k = []
        for i in range(1, 4):
            final_top_k.append(top_k_accuracy_score(
                y_test, y_pred_2, k=i, labels=list(set(y_train))))
    ...
    if outputPath is not None:
        ...
        with pd.ExcelWriter() as writer:
            parameters_table.to_excel(
                writer, sheet_name="Classifier parameters")
            basic_table.to_excel(
                writer, sheet_name="Cross Validation Basic scores")
            balanced_table.to_excel(
                writer, sheet_name="Cross Validation Balanced scores")
            top_1_table.to_excel(
                writer, sheet_name="Cross Validation Top 1 scores")
            top_2_table.to_excel(
                writer, sheet_name="Cross Validation Top 2 scores")
            top_3_table.to_excel(
                writer, sheet_name="Cross Validation Top 3 scores")
            final_test_basic.to_excel(
                writer, sheet_name="Validation set Basic scores")
            final_test_balanced.to_excel(
                writer, sheet_name="Validation set Balanced scores")
            final_test_top_k.to_excel(
                writer, sheet_name="Validation set Top K scores")
...
def cross_validate_multiclass(path2dataset: str, outputPath: str = None, train_ratio=0.75, seed=1, n_splits=5, n_jobs=18, estimator_params_set=1, classification_features_version: str = "v1", stride: int = 15, level: float = None, n_weights: int = 3, weights_preset: int = 1, threshold: float = 0.7, **estkwargs):
    ...
    for m in tqdm(models, desc="Cross validate models"):
        ...
        basic_scores = cross_val_score(
            clf, X_train, y_train, cv=n_splits, n_jobs=n_jobs)
        basic_table[m] = np.append(basic_scores, [np.max(
            basic_scores), basic_scores.mean(), basic_scores.std()])

        balanced_scores = cross_val_score(
            clf, X_train, y_train, cv=n_splits, scoring='balanced_accuracy', n_jobs=n_jobs)
        balanced_table[m] = np.append(balanced_scores, [np.max(
            balanced_scores), balanced_scores.mean(), balanced_scores.std()])

        top_k_scores = cross_validate(
            clf, X_train, y_train, scoring=top_k_scorers, cv=5, n_jobs=n_jobs)
        top_1_table[m] = np.append(top_k_scores['test_top_1'], [np.max(
            top_k_scores['test_top_1']), top_k_scores['test_top_1'].mean(), top_k_scores['test_top_1'].std()])
        top_2_table[m] = np.append(top_k_scores['test_top_2'], [np.max(
            top_k_scores['test_top_2']), top_k_scores['test_top_2'].mean(), top_k_scores['test_top_2'].std()])
...
def calculate_metrics_exitpoints(dataset: str or List[str],
    ...
    start = time.time()
    if not preprocessed:
        tracks = filter_trajectories(
            tracks, threshold, enter_exit_dist, False, detDist=0.1)
    ...
    cluster_labels = labels[labels > -1]
    print("Number of labeled trajectories after clustering: %d" %
          len(tracks_labeled))
    ...
    pooled_cluster_centers = calc_cluster_centers(
        tracks_labeled, reduced_labels)
    ...
    # shuffle tracks, and separate into a train and test dataset
    train, test = train_test_split(
        train_dict, test_size=test_ratio, random_state=42)
    ...
    if feature_version == "1":
    ...
    elif feature_version == "9":
        X_train, y_train, y_reduced_test, metadata_train = FeatureVector.factory_9(
            trackedObjects=tracks_train,
            labels=labels_train,
            pooled_labels=reduced_labels_train,
        )
        X_test, y_test, y_reduced_test, metadata_test = FeatureVector.factory_9(
            trackedObjects=tracks_test,
            labels=labels_test,
            pooled_labels=reduced_labels_test,
        )
    elif feature_version == "10":
        X_train, y_train, y_reduced_test, metadata_train = FeatureVector.factory_10(
            trackedObjects=tracks_train,
            labels=labels_train,
            pooled_labels=reduced_labels_train,
            window_length=7,
        ...
        X_test, y_test, y_reduced_test, metadata_test = FeatureVector.factory_10(
            trackedObjects=tracks_test,
            labels=labels_test,
            pooled_labels=reduced_labels_test,
            window_length=7,
    ...
    original_results = pd.DataFrame(
        columns=['Top-1', 'Top-2', 'Top-3', 'Balanced Accuracy'])
    ...
    for m in models_to_benchmark:
        ...
        for i in range(1, 4):
            ...
            final_top_k.append(top_k_accuracy_score(y_test, y_pred_proba, k=i))
        # add results to table
        original_results.loc[m] = {'Top-1': final_top_k[0], 'Top-2': final_top_k[1],
                                   'Top-3': final_top_k[2], 'Balanced Accuracy': balanced_accuracy}
        print("Classifier %s evaluation based on original clusters: balanced accuracy: %f, top-1: %f, top-2: %f, top-3: %f" %
              (m, balanced_accuracy, final_top_k[0], final_top_k[1], final_top_k[2]))
        # evaluate based on exit centroids
        balanced_accuracy_centroids = balanced_accuracy_score(
            y_reduced_test, y_pred_reduced)
        final_top_k_centroids = []
        for i in range(1, 4):
            final_top_k_centroids.append(top_k_accuracy_score(
                y_reduced_test, y_pred_proba_reduced, k=i, labels=list(set(y_train))))
        # add results to table
        reduced_results.loc[m] = {'Top-1': final_top_k_centroids[0], 'Top-2': final_top_k_centroids[1],
                                  'Top-3': final_top_k_centroids[2], 'Balanced Accuracy': balanced_accuracy_centroids}
        print("Classifier %s evaluation based on exit point centroids: balanced accuracy: %f, top-1: %f, top-2: %f, top-3: %f" %
              (m, balanced_accuracy_centroids, final_top_k_centroids[0], final_top_k_centroids[1], final_top_k_centroids[2]))
        ### Save model if output path is given ###
        if output is not None:
            misclassified = list(
                {metadata_test[i][-1] for i in range(len(X_test)) if y_test[i] != y_pred[i]})
            ...
            for p in paths:
                ...
                save_trajectories(_misclassified, _output,
                                  f"{m}_{str(Path(p).stem)}")
...
def main():
    ...
    argparser = argparse.ArgumentParser(
        "Train, validate and test for renitent detection.")
    argparser.add_argument("--n-jobs", type=int,
                           help="Number of processes.", default=1)

    submodule_parser = argparser.add_subparsers(
        help="Program functionalities.")
    ...
    plot_parser.add_argument(
        "--decision_tree", help="Path to decision tree joblib modell.")
    ...
    exitpoint_metrics_parser.add_argument("--preprocessed", action="store_true",
                                          help="If dataset is already preprocessed, use this flag for faster loading. Default: False")
    exitpoint_metrics_parser.add_argument("--enter-exit-distance", type=float, default=0.4,
                                          help="Euclidean distance threshold for enter and exit points. Default: 0.4")
    exitpoint_metrics_parser.add_argument(
        "--min-samples", default=50, type=int, help="OPTICS clustering param. Default: 50")
    exitpoint_metrics_parser.add_argument(
        "--max-eps", default=0.2, type=float, help="OPTICS clustering param. Default: 0.2")
    exitpoint_metrics_parser.add_argument(
        "--xi", default=0.15, type=float, help="OPTICS clustering param. Default: 0.15")
    exitpoint_metrics_parser.add_argument(
        "-p", "--p-norm", default=2, type=float, help="OPTICS clustering param. Default: 2")
    exitpoint_metrics_parser.add_argument(
        "--mse", default=0.5, type=float, help="Mean squared error threshold for KMeans search. Default: 0.5")
    exitpoint_metrics_parser.add_argument("--models", nargs="+", default=[
                                          "SVM", "KNN", "DT"], help="Models to use for classification. Default: SVM, KNN, DT")
    exitpoint_metrics_parser.add_argument(
        "--background", help="Background image for plots.")
    exitpoint_metrics_parser.add_argument("--feature-version", type=str, default="1", choices=[
                                          "1", "1SG", "7", "7x0.5", "7x2", "7SG", "8", "8SG", "9", "10"], help="Feature Vectors version number. Default: 1")
