...
def predictLinear(trackedObject: TrackedObject, k=3, historyDepth=3, futureDepth=30):
    ...
    if len(x_history) >= 3 and len(y_history) >= 3:
        ...
        if movementIsRight(trackedObject):
            X_test = np.linspace(
                X_train[-1], X_train[-1]+futureDepth, num=futureDepth)
        else:
            X_test = np.linspace(
                X_train[-1], X_train[-1]-futureDepth, num=futureDepth)
    ...
    if len(x_history) >= 3 and len(y_history) >= 3:
        ...
        if movementIsRight(trackedObject):
            ...
            X_test = np.linspace(
                X_train[-1], X_train[-1]-futureDepth, num=futureDepth)
        # poly features
        polyModel = make_pipeline(PolynomialFeatures(degree), linear_model.RANSACRegressor(
            estimator=linear_model.Ridge(alpha=0.5), min_samples=2))
    ...
    if len(x_history) >= 4 and len(y_history) >= 4:
        ...
        if movementIsRight(trackedObject):
            ...
            X_test = np.linspace(
                X_train[-1], X_train[-1]-futureDepth, num=futureDepth)
        # poly features
        splineModel = make_pipeline(SplineTransformer(
            degree=degree, n_knots=n_knots), linear_model.Ridge(alpha=1e-3))
...
def predictLinPoly(trackedObject: TrackedObject, degree=2, k=3, historyDepth=3, futureDepth=30):
    ...
    Args:
        ...
        futureDepth (int, optional): Prediction vectors length. Defaults to 30.
    """
    predictPoly(trackedObject, degree=degree, k=k,
                historyDepth=historyDepth, futureDepth=futureDepth)
    for idx in range(1, len(trackedObject.futureY)-1):
        if ((trackedObject.futureY[idx] > trackedObject.futureY[idx-1] and trackedObject.futureY[idx] > trackedObject.futureY[idx+1]) or
            ...
            predictLinear(trackedObject, k=k,
                          futureDepth=futureDepth, historyDepth=historyDepth)
            break
...
def predictLinSpline(trackedObject: TrackedObject, historyDepth=3, futureDepth=30):
    ...
    Args:
        ...
        futureDepth (int, optional): _description_. Defaults to 30.
    """
    predictSpline(trackedObject, historyDepth=historyDepth,
                  futureDepth=futureDepth)
    for idx in range(1, len(trackedObject.futureY)-1):
        if ((trackedObject.futureY[idx] > trackedObject.futureY[idx-1] and trackedObject.futureY[idx] > trackedObject.futureY[idx+1]) or
            (trackedObject.futureY[idx] < trackedObject.futureY[idx-1] and trackedObject.futureY[idx] < trackedObject.futureY[idx+1]) or
                trackedObject.futureY[idx] < 0):
            predictLinear(trackedObject, futureDepth=futureDepth,
...
def draw_history(trackedObject, image, frameNumber):
    if trackedObject.history[-1].frameID == frameNumber:
        ...
        for det in trackedObject.history:
            if (idx % 1) == 0:
                cv.circle(image, (int(det.X), int(det.Y)),
                          1, color=(0, 255, 0))
            idx += 1
...
def predictWeightedLinPoly(trackedObject: TrackedObject, historyDepth=3, futureDepth=30):
    ...
    train_X, train_y = calcWeigths(
        train_X, trackedObject.AX, train_y, trackedObject.AY)
    if len(train_X) >= 3 and len(train_y) >= 3:
        ...
        if movementIsRight(trackedObject):
            ...
            X_test = np.linspace(X_train[-1], X_train[-1]-futureDepth)
        ##### Polynom fitting #####
        polyModel = make_pipeline(PolynomialFeatures(degree=2), linear_model.RANSACRegressor(
            base_estimator=linear_model.Ridge(alpha=0.5), random_state=30, min_samples=X_train.reshape(-1, 1).shape[1]+1))
        polyModel.fit(X_train.reshape(-1, 1), y_train.reshape(-1, 1))
        # print(X_train.shape, y_train.shape)
        y_pred = polyModel.predict(X_test.reshape(-1, 1))
        for idx in range(1, len(y_pred)-1):
            if ((y_pred[idx] > y_pred[idx-1] and y_pred[idx] > y_pred[idx+1]) or
                ...
                model = linear_model.RANSACRegressor(base_estimator=linear_model.LinearRegression(
                    n_jobs=-1), random_state=30, min_samples=X_train.reshape(-1, 1).shape[1]+1)
                reg = model.fit(X_train.reshape(-1, 1), y_train.reshape(-1, 1))
                y_pred = reg.predict(X_test.reshape(-1, 1))
                break
        trackedObject.futureX = X_test
