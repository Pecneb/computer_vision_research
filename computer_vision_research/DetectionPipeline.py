...
class Yolov7(object):
    ...
    """
    def __init__(self, weights: str, conf_thres: float=0.6, iou_thres: float=0.4, imgsz: int=640, stride: int=32, half: bool=True, device: Union[int, str]=0, batch_size: int=1, debug: bool=True) -> None:
        # region init logger
    ...
    def postprocess(self, pred: torch.Tensor, im0: np.ndarray, im: np.ndarray) -> List:
        ...
        """
        _pred = non_max_suppression(pred, conf_thres=self.conf_thres, iou_thres=self.iou_thres, classes=self.classes)
        detections_adjusted = []
        for det in _pred:
            if len(det):
...
class DeepSORT(object):
    ...
    def __init__(self, max_cosine_distance: float=10.0, max_iou_distance: float=0.7, nn_budget: float=100, historyDepth: int=30, debug: bool=False) -> None:
        ...
        self._Tracker = self.tracker_factory(metric=self.Metric, max_iou_distance=max_iou_distance, historyDepth=historyDepth)
    ...
    def update_history(self, history: List[TrackedObject], new_detections: List[DarknetDetection], joblibbuffer: Optional[List[TrackedObject]]=None, db_connection: Optional[str]=None):
        ...
        self.Tracker.update(wrapped_Detections)
...
class Pipeline:
    ...
    @staticmethod
    def generate_db_path(source: Union[str, Path], outdir: Optional[Union[str, Path]]=None, suffix: str=".joblib") -> Path:
        ...
        _source = Path(source)
        if outdir is None:
            return _source.with_suffix(suffix=suffix)
        return Path(outdir).joinpath(_source.with_suffix(suffix=suffix))
    
    @staticmethod
    def filter_objects(new_detections: List, frame_number: int, names: List[str]=["car"]) -> List[DarknetDetection]:
    ...
    def run(self, yolov7: Yolov7, deep_sort: Optional[DeepSORT]=None, show: bool=False):
        ...
        yolov7 : Yolov7
            _description_
        deep_sort : Optional[DeepSORT], optional
        ...
        for path, img, im0s, vid_cap in self._dataset:
            ...
            # preds = Yolov7.infer(img)
            # preds = Yolov7.postprocess(preds, im0, im)
            if show:
                cv2.imshow(img)
...
if __name__ == "__main__":
    # TODO test runs
    det = Pipeline("yolov7/yolov7.pt", debug=True)
    det.run(show=True)