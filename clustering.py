...
def clustering_on_4D_feature_vectors(estimator, trackedObjects: list[TrackedObject], outdir: str, n_jobs: int = -1, threshold: float = None, **estkwargs):
    ...
    dirpath = os.path.join(outdir, f"{fitted_estimator}_n_clusters-{n_clusters}_feature-vectors-{X.shape[0]}_threshold-{threshold}")
...
def clustering_search(estimator, database: str, outdir: str, threshold: float = (0.1, 0.7), n_jobs: int = -1, **estkwargs):
    # Load tack dataset
